...
| URL                                         | Suggested title                             |
| ------------------------------------------- | ------------------------------------------- |
| `/tax-resolution`                           | `Tax Resolution & IRS Notice Help Katy TX   |
| `/quickbooks-bookkeeping-services`          | `QuickBooks Bookkeeping & Cleanup Katy TX   |
| `/industries`                               | `Industry Tax & Accounting Services         |
| `/new-york/buffalo-tax-accountant`          | `Buffalo Tax Accountant for Businesses      |
| `/pennsylvania-tax-accounting-services`     | `Pennsylvania Tax & Accounting Services     |
| `/pennsylvania/philadelphia-tax-accountant` | `Philadelphia Tax Accountant for Businesses |
| `/pennsylvania/pittsburgh-tax-accountant`   | `Pittsburgh Tax Accountant for Businesses   |
...
# 14. P1: Performance and Core Web VitalsF
...
- [x] Review third-party script cost.
- [x] Load analytics in a way that minimizes main-thread blocking.
...
- [x] Test homepage, contact, calculator, blog, city, and service templates.
- [x] Check font loading and consider self-hosting fonts if appropriate.
- [ ] Confirm no layout shifts from images, embeds, or dynamic content.
- [x] Review map iframe impact on the contact page.
- [x] Check bundle size after each major client component.

T-102 source completion record (July 30, 2026):

- Completed the source audit across the homepage, contact, calculator, blog, city, priority service, and services-hub templates. Evidence: `PERFORMANCE_AUDIT_2026-07-30.md`.
- Deferred the global footer newsletter form until its container approaches the viewport, reducing the shared initial client graph from 103.8 KB to 100.9 KB minified and uncompressed.
- Throttled fixed-navigation scroll work with a passive listener and one animation-frame update at a time.
- Replaced the contact page's initially requested Google Maps iframe with an explicit click-to-load embed and a normal Maps link. Browser verification recorded zero map requests before interaction and one iframe after the load action.
- Verified responsive image dimensions and `sizes`, framework-managed font delivery, route-scoped client bundles, calculator tabs, blog filtering, and representative route output.
- ESLint, TypeScript, and the Next.js production build passed; 71 static pages generated; browser QA found zero page errors, hydration messages, or unexpected internal request failures.
- Vercel Speed Insights, GSC Core Web Vitals, production telemetry delivery, and 14/30/60/90-day LCP, INP, and CLS comparisons remain field-only recurring checks.
